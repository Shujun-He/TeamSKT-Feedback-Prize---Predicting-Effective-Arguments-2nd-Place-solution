...
def run(args, trn_df, val_df, pseudo_df=None):
    ...
    if True:
        ...
        if 'tascj' in args.pretrained_detector_path:
            ...
            model_pretraining.load_state_dict(torch.load(args.pretrained_detector_path))
        elif args.pretrained_mlm_path!='none':
            from models_pretrain_generator import Model as Generator
            model_pretraining = Generator(
                args.model,
                tokenizer,
                num_labels=len(tokenizer), 
            )
            model_pretraining.load_state_dict(torch.load(args.pretrained_mlm_path))
