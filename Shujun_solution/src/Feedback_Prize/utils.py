...
def _prepare_training_data_helper(input_path, tokenizer, df, train_ids):
    training_samples = []
    for idx in tqdm(train_ids):
        ...
        discourse_head = [0]*len(input_labels)
        ...
        for _, row in temp_df.iterrows():
            ...
            discourse_head[targets_start] = 1
            input_labels[targets_start] = prediction_label
            input_labels[targets_start + 1 : targets_end + 1] = [prediction_label] * (targets_end - targets_start)

        sample["input_ids"] = input_ids
        sample["input_labels"] = input_labels
        sample["discourse_head"] = discourse_head
        training_samples.append(sample)
    return training_samples


def prepare_training_data(df, tokenizer, input_path, num_jobs):
    training_samples = []
    ...
    results = Parallel(n_jobs=num_jobs, backend="multiprocessing")(
        delayed(_prepare_training_data_helper)(input_path, tokenizer, df, idx) for idx in train_ids_splits
    )
...
def process_output(output):
    output = (out.cpu().detach().numpy() for out in output)
    return output


def predict(model, dataset, sampler=None, batch_size=16, n_jobs=1, collate_fn=None):
    if next(model.parameters()).device != model.device:
    ...
    for _, data in enumerate(tk0):
        with torch.no_grad():
            out = model.predict_one_step(data)
            out = process_output(out)
            yield out

        if not model.using_tpu:
...
class EarlyStopping(Callback):
    ...
    def on_epoch_end(self, model):
        ...
        for j in range(len(self.valid_samples)):
            ...
            self.valid_samples[j]["discourse_head"] = final_dheads[j][1:]
        ...
        for _, sample in enumerate(self.valid_samples):
            ...
            sample_dhead = sample["discourse_head"]

            # pad preds to same length as offset_mapping
            if len(preds) < len(offset_mapping):
                preds = preds + ["O"] * (len(offset_mapping) - len(preds))
                sample_pred_scores = sample_pred_scores + [0] * (len(offset_mapping) - len(sample_pred_scores))
                sample_dhead = sample_dhead + [0] * (len(offset_mapping) - len(sample_dhead))
            ...
            while idx < len(offset_mapping):
                ...
                while idx < len(offset_mapping):
                    ...
                    if preds[idx] == matching_label and sample_dhead[idx] <= logit_thresh[label]:
