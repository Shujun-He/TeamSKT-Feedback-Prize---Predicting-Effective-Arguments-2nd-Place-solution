...
for essay_id in train_pl['essay_id']:
    with open(os.path.join("../../../Feedback_Prize/input/train",essay_id+'.txt'),'r') as f:
        text=f.read()
...
val_loss, oof, pred_prob_sequences = val(0)
with open(f"{args.experiment_name}/oofs/{args.fold}_{args.max_len}.p",'wb+') as f:
    pickle.dump(pred_prob_sequences,f)
oof.to_csv(f"{args.experiment_name}/oofs/{args.fold}_{args.max_len}.csv",index=False)
